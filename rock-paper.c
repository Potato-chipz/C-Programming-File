...
void * thread_player(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++ ) {
        ...
        pthread_barrier_wait(&result->barrier); //referee wait til both players sent the result

        //get the result from referee and save it into outcome 
        pthread_mutex_lock(&choice->mutex); 

        while(choice->status != S_RESULT)
        {
...
void * thread_referee(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++) {
        ...
        pthread_barrier_wait(&result->barrier); //wait till both players moves have been recorded and sent to referee

        //compare choice 
        outcome = compare_choices(choice1,choice2); 

        //store the outcome into result and notify each player the result is ready 
