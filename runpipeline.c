...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    //making sure the current porcess is valid 
    if (cur >= num_programs) 
    {
        die("Invalid Program Index!");
    }

    //create an insteance of currnet process 
    Program *current_program = &programs[cur];
    ...
    if (pid == 0) {
        ...
        //redirect file directory, get the input from the last input 
        if (cur > 0) 
        {
            dup2(programs[cur - 1].fd_out, FD_STDIN);
            close(programs[cur - 1].fd_out);
        }

        //redirect file directory 
        if (cur < num_programs - 1) 
        {
            dup2(current_program->fd_out, FD_STDOUT);
            close(current_program->fd_out);
        }

        // Close all file descriptors that are not needed in the child process
        ...
        // Set the child process's standard input and output as specified
        if (current_program->fd_in != -1) 
        {
            dup2(current_program->fd_in, FD_STDIN);
            close(current_program->fd_in);
        }

        if (current_program->fd_out != -1) 
        {
            dup2(current_program->fd_out, FD_STDOUT);
            close(current_program->fd_out);
        }
        ...
        execvp(current_program->argv[0], current_program->argv);

        // If execvp fails, report the error
        die("execvp() failed");
        exit(-1);
    }
    else //in the parent porcess
    {
        // Close file descriptors not needed for the communication
        if (cur > 0) {
            close(programs[cur - 1].fd_out);
            close(programs[cur - 1].fd_in);
        }

        //if the current porcess is not the last 
        if (cur < num_programs - 1) 
        {
            close(current_program->fd_out);
            close(current_program->fd_in);
        ...
        //close the file descriotor for the last process
        close(current_program->fd_out);
        close(current_program->fd_in);

        // Save the child process ID in the current_program
        current_program->pid = pid;
    }
...
int wait_on_program(Program *prog)
{
    ...
    // TODO
    //if the child has not finished the process
    if(waitpid(prog->pid,&exitStatus,0)== -1)
    {
        die("waitpid() fail!");
    }

    //if the child has finished the processs 
...
{
    // TODO
    int pipes[MAX_NUM_PROGRAMS - 1][2]; // Array to store pipe file descriptors, there are 6 pipes in total 

    // Create pipes for communication between programs
    ...
    // Assign pipe file descriptors to the corresponding programs
    for (int i = 0; i < num_programs - 1; i++) {
        programs[i].fd_out = pipes[i][PIPEFD_WRITE];
        programs[i + 1].fd_in = pipes[i][PIPEFD_READ];
    }
}
